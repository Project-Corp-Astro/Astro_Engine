...
# //////////////////////////////////////////////////////////////
#  # End of AstroCeleryManager class with out redis.
# Celery Task Queue Manager for Astro Engine
# Phase 4.1: Celery Setup and Framework
...
class AstroCeleryManager:
    ...
    def init_app(self, app):
        """Initialize Celery with Flask app (no Redis)"""

        # Configuration - broker/backend can be set via environment or use default (no Redis)
        broker_url = os.getenv('CELERY_BROKER_URL', 'pyamqp://guest@localhost//')
        result_backend = os.getenv('CELERY_RESULT_BACKEND', 'rpc://')

        # Create Celery instance (no Redis in broker/backend)
        self.celery_app = Celery(
            'astro_engine',
            broker=broker_url,
            backend=result_backend,
    ...
    def get_queue_stats(self) -> Dict[str, Any]:
        """Get queue statistics and monitoring info (no Redis queue length)"""
        if not self.celery_app:
            return {'error': 'Celery not available', 'success': False}

        try:
            inspect = self.celery_app.control.inspect()

            # Active, scheduled, reserved tasks (may be None if no workers)
            active_tasks = inspect.active() or {}
            scheduled_tasks = inspect.scheduled() or {}
            reserved_tasks = inspect.reserved() or {}

            # No Redis queue length; only task counts from Celery
            recent_tasks = list(self.task_results.values())[-10:]  # Last 10 tasks
            ...
            stats = {
                ...
                'queue_stats': {
                    ...
                    'scheduled_tasks': sum(len(v) for v in scheduled_tasks.values()) if scheduled_tasks else 0,
                    'reserved_tasks': sum(len(v) for v in reserved_tasks.values()) if reserved_tasks else 0,
                },
                'recent_tasks': recent_tasks,
                'celery_config': {
                    'broker': self.celery_app.conf.broker_url,
    ...
    def cleanup_completed_tasks(self, max_age_hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error("❌ Failed to cleanup tasks", error=str(e))
            return {
                'success': False,
                'error': str(e)
            }
